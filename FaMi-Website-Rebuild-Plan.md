...
> **Session note (Aug 19, 2026):** The codebase export this plan was checked
> against was missing its entire foundation layer (`lib/db`, `lib/auth`,
> `lib/data`, `context`, `types`, all `components/ui|layout|shop`, root
> `layout.tsx`, `globals.css`) despite Phase 3 being marked done below — it's
> been rebuilt to match the existing page/component contracts, verified with
> a real `next build` + `next start` smoke test (register/login/checkout/
> wishlist/stock-rollback), and a few real bugs fixed along the way (checkout
> race condition on stock, non-atomic loyalty points, unhandled JSON parse
> errors, a `useSearchParams` prerender crash on `/login`). See
> `fami-app/README.md` for the full list of what changed and what's still
> open (admin panel, Cloudinary wiring, order-notification email, SEO,
> analytics). Checkboxes below are left as-is for history; treat the README
> as the current source of truth on what's actually implemented.
